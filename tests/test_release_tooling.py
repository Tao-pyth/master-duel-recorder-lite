...
class ReleaseToolingTest(unittest.TestCase):
    def test_windows_version_resource_uses_project_version(self) -> None:
        ...
        self.assertEqual(windows_version_tuple(version), (0, 13, 1, 0))
        self.assertIn("filevers=(0, 13, 1, 0)", resource)
        self.assertIn("ProductVersion', '0.13.1'", resource)
    ...
    def test_release_tag_matches_both_version_sources(self) -> None:
        self.assertEqual(read_package_version(), "0.13.1")
        self.assertEqual(verify_project_version(), "0.13.1")
        self.assertEqual(verify_release_tag("v0.13.1"), "0.13.1")

    def test_release_tag_script_supports_direct_execution(self) -> None:
        completed = subprocess.run(
            [sys.executable, "scripts/verify_release_tag.py", "v0.13.1"],
            cwd=PROJECT_ROOT,
            check=False,
            capture_output=True,
            text=True,
        ...
        self.assertEqual(completed.stdout.strip(), "0.13.1")
