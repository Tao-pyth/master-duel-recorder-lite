...
HISTORY_OPTIONAL_COLUMNS = ("coin_face", "duel_type", "duration", "size")
COLOR_KEYS = (
    "result.win",
    "result.loss",
    "result.draw",
...
class UiPreferences:
    history_visible_columns: tuple[str, ...] = HISTORY_OPTIONAL_COLUMNS
    ...
    def normalized(self) -> UiPreferences:
        columns = tuple(
            name for name in HISTORY_OPTIONAL_COLUMNS if name in self.history_visible_columns
        )
        colors = {
        ...
        return UiPreferences(columns, colors, bool(self.automatic_update_check))
...
def load_ui_preferences(config_directory: Path) -> UiPreferences:
    ...
    try:
        ...
        if not isinstance(document, dict):
            return UiPreferences()
        raw_columns = document.get("history_visible_columns", HISTORY_OPTIONAL_COLUMNS)
        raw_colors = document.get("history_cell_colors", {})
        return UiPreferences(
            tuple(str(item) for item in raw_columns)
            if isinstance(raw_columns, list)
            else HISTORY_OPTIONAL_COLUMNS,
