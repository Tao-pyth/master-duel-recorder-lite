...
現在のバージョンは `0.17.1`、「録画・対戦情報の統合管理」です。デッキ名とタグを個別画面で管理でき、説明、タグカラー、安定IDを保持します。GUIから録音元を選択・テストでき、録画履歴は録画IDではなく勝敗、先後、対戦種別を中心に表示します。Master Duel日本語UIの実画面特徴からコイントス、盤面、ターン、勝敗、マッチエラー、リプレイを区別し、自動録画とタイムラインへ接続します。外部サービスへの直接アップロードとOAuthは実装していません。V1.0.0への更新はユーザーの明示指示を待ちます。
