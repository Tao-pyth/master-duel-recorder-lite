...
現在の開発バージョンは `0.19.0`、「シーズン管理」です。シーズンごとの戦績とメモ、録画履歴のシーズン・自分デッキ・相手デッキ・タグ絞り込み、デッキカラーと用途フラグを提供します。統計にはカレンダー入力、先後別勝率、デッキ別全体、デッキ先後別を統合しました。信頼性と管理負担の見直しにより復旧機能は撤去しています。外部サービスへの直接アップロードとOAuthは未実装で、V1.0.0への更新はユーザーの明示指示を待ちます。
