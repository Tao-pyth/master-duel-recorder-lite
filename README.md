...
現在のバージョンは `0.16.4`、「Windows FFmpeg起動安定化」です。GitHub Releaseでは、通常利用向けGUI EXEと自動化・詳細操作向けCLI EXEを配布します。WindowsでFFmpeg子プロセスを非表示起動し、一時的なDLL初期化失敗`0xc0000142`だけを1回再試行します。開始前診断に失敗した場合は項目と理由をGUIへ表示します。自動監視はMaster Duelウィンドウが表示されただけでは録画せず、開始演出から盤面への遷移を複数フレームで確認してから録画します。外部サービスへの直接アップロードとOAuthは実装していません。V1.0.0への更新はユーザーの明示指示を待ちます。
...
既定では`masterduel.exe`の可視・非最小化ウィンドウを固定し、最大2fpsで開始演出から盤面への遷移を3フレーム合意すると録画します。メニュー、デッキ編集、設定画面の表示だけでは開始しません。録画開始前の画像はメモリだけで扱い、開始候補を録画タイムラインの0秒へ保存します。録画後はウィンドウ不在、最小化、対象変更を5回連続で確認すると停止します。Ctrl+Cは監視と実行中の録画を正常停止します。`record`と`watch`はOSロックにより同時録画を拒否します。詳細は[Master Duel向け録画補助設計](docs/architecture/detection.md)と[基本イベント自動判定設計](docs/architecture/visual-event-detection.md)を参照してください。
