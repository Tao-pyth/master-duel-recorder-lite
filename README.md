...
現在のバージョンは `0.16.2`、「対戦開始基準の自動録画」です。GitHub Releaseでは、通常利用向けGUI EXEと自動化・詳細操作向けCLI EXEを配布します。自動監視はMaster Duelウィンドウが表示されただけでは録画せず、開始演出から盤面への遷移を複数フレームで確認してから録画します。録画中はターン切り替え、勝敗・対戦終了を未確認候補としてタイムラインへ保存します。外部サービスへの直接アップロードとOAuthは実装していません。V1.0.0への更新はユーザーの明示指示を待ちます。
