...
現在の開発バージョンは `0.19.1`、「GUI一貫性とデータ管理改善」です。シーズン管理に加え、主要な入力欄・プルダウン・ボタンの高さと配色を統一し、録画履歴へ自分デッキ名とデッキカラーを表示します。シーズンは日本語種別と期間を一覧上部で直接登録し、独立したレポート画面でライブ集計とメモを確認できます。設定画面では履歴・デッキ・タグ・シーズンをJSONへ入出力でき、二段階確認とSQLiteバックアップを伴う個別初期化を提供します。履歴初期化で録画ファイル自体は削除しません。外部サービスへの直接アップロードとOAuthは未実装で、V1.0.0への更新はユーザーの明示指示を待ちます。
...
シーズンを選択すると、勝率、対戦数、勝敗、先攻・後攻、デッキ別、日・週・月の推移区間を最新データから集計します。統計値のスナップショットは保存せず、レポートメモだけを保存します。統計ページからはシーズンと「未設定」を条件にできます。詳細は[シーズン管理設計](docs/architecture/seasons.md)を参照してください。
