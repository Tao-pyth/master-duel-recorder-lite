...
V0.25.0は未公開のRelease候補です。SHA-256で固定した3440x1440動画14本と1920x1080動画4本の2fps回帰、Ruff、全435テスト、CLI/GUI EXEビルドと両スモークは合格しています。最新fix340 GUIによる初期3戦は合格しましたが、別セッションの本検証で召喚カットインをLOSEと誤認して決着前停止する不具合を確認しました。Issue #367の修正と、修正版GUIによる初期3戦・10戦連続実戦試験、push後のWindows CIが残る公開条件です。外部サービスへの直接アップロードとOAuthは未実装で、V1.0.0への更新はユーザーの明示指示を待ちます。
