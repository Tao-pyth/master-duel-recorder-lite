...
- GUI非依存のレビュー用ViewModel、通常GUI内動画レビュー、PySide6通常配布GUI

## 現在の状態

現在の正式版は `2.7.14`、「デッキ・タグ一覧の名前可読性修正」です。`master-duel-recorder-lite-gui.exe`はPySide6 GUIを起動し、録画、戦績管理、統計、デッキ名、タグ、シーズン、YouTubeテンプレート、設定、データ保全、CSV/更新、主要ダイアログ、動画レビューの状態を管理・表示できます。信頼性確認は主要ナビではなく設定内の「録画診断・信頼性」へ統合し、MP4準備はYouTube投稿時の内部処理として扱います。既存DB、戦績、録画履歴、デッキ、タグ、シーズン、設定、バックアップ操作を更新後も通常GUIから扱えることを優先します。

V2.7.11では、シーズンの「レポート」から概要・メモ、内訳、日別/週別推移を閲覧し、HTML保存できるようにしました。保存時にも再集計し、既存ファイルの上書きは確認します。[操作仕様](docs/architecture/pyside-season-report.md)を参照してください。

V2.7.10では、音声候補の更新後も選択と未保存内容を保持し、音声モードと入力を同時に正しく保存できるようにしました。[操作と保存仕様](docs/architecture/audio-settings-integrity.md)を参照してください。

V2.7.14では、デッキ・タグの色と説明を名前の列にまとめ、長い説明が名前の幅を奪わないようにしました。名前は1行で表示し、全文はツールチップと右側編集欄で確認できます。

V2.7.13では、デッキ名・タグ・シーズンを左の検索一覧と右側編集へ統一しました。新規と保存を分け、未保存入力を保護します。[操作説明](docs/architecture/pyside-catalog-editors.md)を参照してください。

V2.7.12では、戦績管理を上部の絞り込み・全幅一覧・下部編集へ再構成しました。「未完了のみ」は再クリックで解除でき、他の条件を保持します。未保存の入力を保護し、複数選択は一括編集へつながります。[操作説明](docs/architecture/pyside-history-redesign.md)を参照してください。

V2.7.9では、統計の実数値と条件反映、録画操作の横見切れ、設定の共通保存と未保存保持、戦績一覧の未完了表示と配置、グラフ数値、案内文を改善しました。操作方法は[GUI品質修正](docs/architecture/gui-audit-fixes.md)を参照してください。

V2.7.7では、録画画面から自動監視で作成する戦績下書きの初期値として、自分デッキ、シーズン、希望先後を選べるようにしました。自動監視開始時点の値を固定し、成功した新規自動録画の戦績下書きだけへ反映します。検出された実際の先後は希望値より優先し、希望先後と実際の先後が一致した場合はコイン表、不一致の場合は裏として補助入力します。実先後が不明な場合はコインを未設定のままにし、手動録画、録画なし手動戦績、既存戦績、SQLite schema、録画ファイル、queue、manifest、OAuth資格情報は変更しません。

V2.7.0では、アプリ内レビュー画面に視覚タイムラインMVPを追加しました。録画長に対する現在位置、選択位置、タイムラインイベント位置を動画の流れに沿って確認でき、視覚タイムラインのイベント選択と表形式タイムラインの選択は既存の動画シークへ同期します。`duel_start`、手動マーカー、候補イベント、通常イベントは表示種別を分け、録画長不明や範囲外イベントでも保存データを変更しません。クリップ範囲エディタ、波形/サムネイル、DB schema、設定形式、OAuth、queue、manifestは変更しません。

V2.6.0では、自動録画だけを対象に短いプリロールを追加しました。既定では無効で、設定画面またはCLIで明示的に有効化した場合だけ、対戦開始判定前の短い一時映像を録画へ含めます。一時バッファは `user_data/` 配下に保存し、秒数と容量の上限で増え続けないようにします。プリロール結合に失敗した場合も通常の自動録画を可能な範囲で残し、失敗理由を診断へ記録します。手動録画、長時間バックグラウンド録画、任意クリップ保存、DB schema、OAuth、queue、manifestは変更しません。

V2.5.0では、PySide6通常GUIの主要画面に表示される操作を棚卸しし、録画対象更新/保存、環境診断保存/フォルダ表示、録画なし戦績追加、クリーンアンインストール、レビュー画面の外部再生/マーカー/クリップ失敗表示を、実処理または具体的な案内へ接続しました。レビュー画面のタイムラインは内部キーではなく日本語の種別、状態、由来を表示します。DB schema、設定形式、録画、queue、manifest、OAuth資格情報は変更しません。

V2.4.1では、テンプレート画面からMP4準備一覧、準備一覧更新、バックグラウンド処理表示を外し、投稿テンプレート編集へ責務を絞りました。戦績管理フィルターは期間、シーズン、デッキ、タグ、コイン、登録元を一覧クエリへ反映し、保存済み条件では既存の複数タグ条件も復旧できます。設定の「表示」は「戦績表示設定」へ改名し、カラー表示はデッキ名、タグ、戦績表示設定でスウォッチ基準へ揃えました。DB schema、設定形式、録画、queue、manifest、OAuth資格情報は変更しません。

V2.4.0では、戦績管理の録画再生ボタンから通常GUI内レビュー画面を開けるようにしました。`.mp4`と`.mkv`はQt Multimediaでアプリ内再生し、再生/一時停止、シーク、現在位置表示、タイムライン一覧、イベントクリックによるシーク、現在位置マーカー追加、選択位置中心のクリップ出力を扱えます。Qt Multimediaが利用できない、または再生でエラーになった場合は外部プレイヤーへfallbackします。DB schema、設定形式、録画、queue、manifest、OAuth資格情報は変更しません。

V2.3.0では、左下の固定「要確認」と録画画面の固定シーズン読み込み表示を実診断・開催中シーズン表示へ接続しました。デッキ名、タグ、シーズンは一覧選択から入力欄へ反映し、追加、保存、削除またはアーカイブを既存サービス層で実行できます。テンプレート画面は投稿テンプレート編集と投稿準備キューへ責務を絞り、YouTube接続管理は設定画面へ集約しました。信頼性画面の状態更新、初回導入確認、YouTube投稿のバックグラウンド実行状態、ボタン/入力/プルダウンの高さ統一、カラー列の色見本表示をGUI smokeと回帰テストで確認します。DB schema、設定形式、録画、queue、manifest、OAuth資格情報は変更しません。

V2.2.3では、戦績管理画面の主要ボタンを実処理、確認ダイアログ、または明確な案内へ接続し、行選択が必要な操作は未選択時に無効化するようにしました。一覧の勝敗、先後、コイン、対戦種別、登録元は内部コードではなく日本語で表示し、期間フィルターはカレンダーポップアップ付きの日付入力で指定できます。主要操作、補助操作、アイコン操作、危険操作の見た目も区別し、GUI smokeと回帰テストでボタン接続、日本語表示、日付ピッカー、選択状態を確認します。DB schema、設定形式、録画、queue、manifest、OAuth資格情報は変更しません。
...
GUIでは録画履歴を選択して「タイムライン」を開き、状態・種別での絞り込み、手動マーカー追加、自動判定候補の確認・却下を行います。レビュー画面では同じタイムラインイベントを視覚タイムライン上にも表示し、動画の現在位置、選択位置、イベント位置を合わせて確認できます。CLIでは録画開始からの経過ミリ秒を指定します。
