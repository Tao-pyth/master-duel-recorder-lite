...
現在の開発バージョンは `0.21.2`、「初期画面・戦績更新排他」です。録画とは独立した`duel_id`で録画あり・なしの対戦を戦績管理ページへ一元表示し、GUIから手動戦績を追加・編集・削除できます。録画操作を初期画面に維持し、開催中シーズン最大2件の勝率要約と手動追加ボタンを配置しました。自動監視中は新規入力を禁止し、既存戦績は読み取り専用で閲覧できます。外部サービスへの直接アップロードとOAuthは未実装で、V1.0.0への更新はユーザーの明示指示を待ちます。
