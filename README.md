...
現在の正式版は `1.4.3`、「アプリ内自動更新updater化」です。V1.0.0の中核機能に加え、手動戦績の削除、FFmpeg・データ保存先の選択、シーズン別統計、表示列とセル色の設定、更新確認、専用アプリアイコン、未完了戦績の連続入力、相手デッキ列、ダブルクリック動作設定、カレンダー表示修正、固定択一項目のボタン選択、YouTube投稿管理、自動録画の信頼性確認、改善支援、GUIからのYouTube連携とprivate投稿導線、更新取得後のGUI EXE起動検証、専用updaterによる自動更新適用を整備しました。
...
- V1.4.2の配布ビルドでは、更新されたYouTube OAuth `client_id`だけをGitHub Secretから同梱します
