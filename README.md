...
現在のバージョンは `0.16.8`、「録画開始中のGUI応答維持」です。GitHub Releaseでは、通常利用向けGUI EXEと自動化・詳細操作向けCLI EXEを配布します。自動監視は開始演出を見逃した場合や対戦途中に開始した場合でも、結果画面や大きな演出ではない高信頼度の対戦盤面を3フレーム連続で確認すると録画します。待機中のアクティビティは`対戦開始を判定中です (4s)`のように経過秒付きの1行で更新します。録画開始の環境診断やFFmpeg準備はGUIを停止させず、処理中の重複操作を明示的に拒否します。Windowsでは診断・画面判定・録画用FFmpegを非表示で起動し、一時的なDLL初期化失敗`0xc0000142`だけを1回再試行します。外部サービスへの直接アップロードとOAuthは実装していません。V1.0.0への更新はユーザーの明示指示を待ちます。
