...
現在のバージョンは `0.16.3`、「FFmpeg 9能力判定修正」です。GitHub Releaseでは、通常利用向けGUI EXEと自動化・詳細操作向けCLI EXEを配布します。FFmpeg 9で追加された入力方式一覧のdeviceフラグ列に対応し、自動導入したFFmpegが持つ`gdigrab`を正しく判定します。自動監視はMaster Duelウィンドウが表示されただけでは録画せず、開始演出から盤面への遷移を複数フレームで確認してから録画します。外部サービスへの直接アップロードとOAuthは実装していません。V1.0.0への更新はユーザーの明示指示を待ちます。
