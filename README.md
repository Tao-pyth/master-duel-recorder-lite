...
現在の正式版は `2.1.0`、「PySide6標準機能移植」です。`master-duel-recorder-lite-gui.exe`はPySide6 GUIを起動し、録画、戦績管理、統計、デッキ名、タグ、シーズン、YouTube、MP4準備、信頼性、設定、データ保全、CSV/更新、主要ダイアログの状態を管理・表示できます。既存DB、戦績、録画履歴、デッキ、タグ、シーズン、設定、バックアップ操作を更新後も通常GUIから扱えることを優先します。

V2.1.0では、PySide6 smokeの15標準機能ゲートを全通過させ、要求widget 51個と主要操作チェックをPySide6側へ揃えました。統計の推移単位はPySide6統計画面の「勝利数・勝率推移」内条件として扱い、既定値を日単位にします。V2.1.0ではDB schema、設定形式、録画、queue、manifest、OAuth資格情報を変更しません。
