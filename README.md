...
現在の正式版は `2.0.2`、「15標準機能の実操作確認」です。V2.0.0で通常入口になったPySide6 GUIは、1.x相当の標準操作面を満たすまで通常入口から外し、`master-duel-recorder-lite-gui.exe`はTkinter GUIを起動します。既存DB、戦績、録画履歴、デッキ、タグ、シーズン、設定、バックアップ操作を更新後も通常GUIから扱えることを優先します。

V2.0.2では、1.x相当の15標準機能を「画面部品が存在する」だけでなく「ユーザーが実操作として到達できる」状態で確認します。GUI smokeと回帰テストの実操作チェックを強化し、その結果を録画後ワークフロー、データ保全表示、PySide6通常入口化ゲートへ反映しました。V1.4.x open Issue棚卸しでも、V2.0.2のBackward Compatibilityを妨げる直接ブロッカーは確認していません。V2.0.2ではPySide6を通常配布入口へ戻さず、DB schema、設定形式、録画、queue、manifest、OAuth資格情報も変更しません。
