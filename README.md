...
現在の正式版は `2.0.3`、「戦績管理・統計・シーズンレポートの読み取り品質改善」です。V2.0.0で通常入口になったPySide6 GUIは、1.x相当の標準操作面を満たすまで通常入口から外し、`master-duel-recorder-lite-gui.exe`はTkinter GUIを起動します。既存DB、戦績、録画履歴、デッキ、タグ、シーズン、設定、バックアップ操作を更新後も通常GUIから扱えることを優先します。

V2.0.3では、戦績管理の長い日本語デッキ名を読みやすくし、統計の推移グラフを「期間ごとの勝利数」と「そこまでの累積勝率」の組み合わせへ変更しました。シーズンレポートでは全体行を比較基準として追加し、0件の未設定行や最終勝敗の重複内訳を標準表示から外します。V2.0.3ではPySide6を通常配布入口へ戻さず、DB schema、設定形式、録画、queue、manifest、OAuth資格情報も変更しません。
