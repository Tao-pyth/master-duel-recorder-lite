...
V0.25.0は未公開のRelease候補です。SHA-256で固定した3440x1440動画14本と1920x1080動画4本の2fps回帰、Ruff、全435テスト、CLI/GUI EXEビルドと両スモークは合格しています。2セッションの実戦ログで、結果を見逃した対戦の録画が次対戦境界まで継続し、次録画の冒頭が欠ける問題を確認して修正しました。最新fix340 GUIによる初期3戦は開始・盤面確定・結果停止3/3で合格しました。残る公開条件は別セッションでの10戦連続実戦試験と、push後のWindows CIです。外部サービスへの直接アップロードとOAuthは未実装で、V1.0.0への更新はユーザーの明示指示を待ちます。
