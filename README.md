...
現在の正式版は `1.0.0`、「中核機能完成」です。録画環境の初期化から自動・手動録画、戦績管理、タイムライン、統計、シーズン、データ保全、単体音声、CSV移行、アップロード準備、クリーンアンインストールまでを一つのWindowsアプリケーションとして提供します。V0.26.2のV1移行監査とクリーン環境E2Eを完了し、ユーザーの明示指示に基づいて正式版へ更新しました。

CSV移行は適用前バックアップと単一DBトランザクションで既存データを保護します。単体音声はWindows build 20348以上が対象で、非対応・ゲーム未起動・ヘルパー障害時には別音源へ無断で切り替えず、警告を残して映像のみ継続します。外部サービスへの直接アップロードとOAuthはV1.0.0の対象外で、アップロード準備までを提供します。
