...
現在の正式版は `2.7.13`、「デッキ名・タグ・シーズンの一覧と右側編集」です。`master-duel-recorder-lite-gui.exe`はPySide6 GUIを起動し、録画、戦績管理、統計、デッキ名、タグ、シーズン、YouTubeテンプレート、設定、データ保全、CSV/更新、主要ダイアログ、動画レビューの状態を管理・表示できます。信頼性確認は主要ナビではなく設定内の「録画診断・信頼性」へ統合し、MP4準備はYouTube投稿時の内部処理として扱います。既存DB、戦績、録画履歴、デッキ、タグ、シーズン、設定、バックアップ操作を更新後も通常GUIから扱えることを優先します。
