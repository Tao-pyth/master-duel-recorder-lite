...
現在の開発バージョンは `0.19.2`、「統計カレンダーピッカー表示修正」です。統計ページの開始日・終了日は、直接入力に加えて右端のカレンダーアイコンから日付を選択できます。狭いウィンドウでもアイコンと条件適用・クリア操作が欠けないよう、フィルター行の幅配分を調整しています。シーズン管理、GUI一貫性、管理データのJSON入出力と安全な個別初期化も引き続き提供します。履歴初期化で録画ファイル自体は削除しません。外部サービスへの直接アップロードとOAuthは未実装で、V1.0.0への更新はユーザーの明示指示を待ちます。
