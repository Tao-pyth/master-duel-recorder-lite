...
現在の開発バージョンは `0.21.1`、「録画なし戦績の事後入力」です。録画とは独立した`duel_id`を全対戦へ発行するDB v10へ移行し、録画を伴わない対戦も同じ対戦記録、変更監査、統計、管理データ入出力で扱う基盤を実装しました。CLIでは`duel create`から手入力できます。録画履歴を戦績管理ページへ統合するGUIと事後入力画面は実装継続中です。外部サービスへの直接アップロードとOAuthは未実装で、V1.0.0への更新はユーザーの明示指示を待ちます。
