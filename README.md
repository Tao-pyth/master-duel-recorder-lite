...
現在のローカル開発バージョンは `0.26.0`、「単体音声・戦績CSV移行」です。V0.25.0までの累積機能に加え、Windows Process LoopbackによるMaster Duel単体音声、4つの音声モード、監視中の音声事前待機、固定11列の戦績CSV入出力を実装しています。詳細は[音声入力設計](docs/architecture/audio-input.md)と[戦績CSV設計](docs/architecture/duel-csv.md)を参照してください。

V0.26.0のCSV移行は適用前バックアップと単一DBトランザクションで既存データを保護します。単体音声はWindows build 20348以上が対象で、非対応・ゲーム未起動・ヘルパー障害時には別音源へ無断で切り替えず、警告を残して映像のみ継続します。外部サービスへの直接アップロードとOAuthは未実装で、V1.0.0への更新はユーザーの明示指示を待ちます。
