...
try {
    ...
    if ($result.operational_quality_audit_contract.missing_action_widgets.Count -ne 0 -or $result.operational_quality_audit_contract.placeholder_only_actions.Count -ne 0) {
        throw "GUI smoke operational quality action contract is invalid"
    }
    if (-not $result.operational_quality_audit_contract.review_timeline_localized -or -not $result.operational_quality_audit_contract.primary_internal_values_hidden) {
        throw "GUI smoke operational quality display contract is invalid"
    }
    foreach ($widget in @("record_target_refresh", "record_target_save", "record_manual_duel_add", "record_diagnostics_export", "clean_uninstall")) {
        if ($result.operational_quality_audit_contract.action_widgets.record -notcontains $widget -and $result.operational_quality_audit_contract.action_widgets.settings -notcontains $widget) {
            throw "GUI smoke operational action is missing: $widget"
        }
    }
