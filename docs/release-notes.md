...
- V0.23.0の結果判定回帰修正を含め、Ruff、単体・統合テスト412件（skip 2件）、CLI/GUI EXEビルド、両EXEのV0.25.0隔離スモークを通過した
...
- 実行時録画による補足評価は合格したが、提供動画14本そのものの再評価とV0.23.0 EXEでの初期3戦・10戦連続試験が未達のため、タグ・Release・`main` pushは保留する
