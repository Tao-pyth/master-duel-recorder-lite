...
- 復元困難な旧提供動画への依存を廃止し、現存する有効11戦・負例3本をサイズとSHA-256で固定した正式な超横長回帰コーパスへ移行した。1920x1080の最新実動画不足は既知リスクとして維持する
...
- fix337 GUIによる初期3戦・10戦連続試験と、その後のWindows CIが未完了のため、タグ・Release・`main` pushは保留する
...
- 現存する有効11戦・負例3本をサイズとSHA-256で固定した正式な超横長回帰コーパスへ移行し、2fps評価に合格した。1920x1080の最新実動画不足は既知リスクとして明記し、fix337 GUIでの初期3戦・10戦連続試験が未達のため、タグ・Release・`main` pushは保留する
