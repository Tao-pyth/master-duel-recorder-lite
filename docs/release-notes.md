...
## V2.7.14: デッキ・タグ一覧の名前可読性修正 - 2026-09-12

- 色と説明を名前の列へ統合し、説明は小さい下段表示とする
- 名前の最低幅を確保し、長い説明が列を押し広げる問題を修正する
- 名前を折り返さず、省略された全文はツールチップと編集欄で確認できる
- 使用回数・用途・履歴/統計はコンパクトな列とし、絞り込みの対象も明示する
- 32件以上の長文データと小画面/拡大表示で検証する。保存値・検索・既存編集機能は維持する
- 追跡: #648 / Milestone V2.7.14

## V2.7.13: デッキ名・タグ・シーズンの一覧と右側編集 - 2026-09-12

- 3ページを検索一覧と右側編集に統一し、新規追加と既存保存を分離する
- 行/検索/条件/新規/ページ移動/終了の未保存入力を保護し、保存失敗時も維持する
- デッキ/タグの全設定、シーズンの期間・種別・アーカイブ・レポートを維持する
- DB/設定形式と録画方式は変更しない
- 追跡: #645、#646、#647 / Milestone V2.7.13

## V2.7.12: 戦績管理の下部編集と操作整理 - 2026-09-12

- 採用3案目に沿い、上部の全体操作・絞り込み、全幅一覧、選択行の下部編集へ再構成する
- 「未完了のみ」を再クリックで解除でき、期間・デッキ・保存済み条件・詳細条件を保持する
- デッキ・勝敗・先後・コイン・確認状態と詳細項目を下部で保存する。複数選択時は一括編集へ切り替える
- 未保存の行移動・条件変更・更新・画面移動・終了を保存/破棄/取消で保護し、保存失敗/競合で入力を失わない
- 再生/レビュー・YouTube・削除・重複・列説明・表示列は対象に応じたその他メニューから到達できる
- 録画/DB/設定形式は変更せず、既存の確認状態とrevision競合検出を維持する
- 追跡: #642、#643、#644 / Milestone V2.7.12。[Release](https://github.com/Tao-pyth/master-duel-recorder-lite/releases/tag/v2.7.12)
...
- 戦績管理の「再生」と「編集」から開いたレビュー画面の保存完了通知を、親の戦績管理表更新へ接続した
- 保存失敗時は親表を更新せず、既存のエラー表示を維持するようにした
...
- 戦績編集ダイアログの自分デッキ/相手デッキ入力を、登録済みデッキ候補を選べて自由入力もできる形式へ戻した
- 戦績編集ダイアログを広げ、保存/キャンセルの表記を日本語化した
- レビュー画面を「マーカー編集」「戦績入力」タブへ拡張し、動画確認中に戦績を保存できるようにした
- レビュー画面から候補タイムラインイベントを確定/却下できるようにした
- 手動マーカーを種別と説明に分けて編集できるようにし、自動判定ラベルは通常編集から保護した
- レビュー表から内部管理用の「由来」列を外し、ユーザー向けには経過、種別、状態、説明へ整理した
- 「選択位置をクリップ出力」は前30秒・後30秒の範囲説明を実行前/完了時に表示し、出力後に保存先をエクスプローラーで開けるようにした
- 主要操作ボタンの線形アイコンを、Pictogrammers Material Design Iconsの考え方に合わせて統一し、primary/dangerボタンでも視認できる色に調整した
...
- V2.7.2実機検証で確認した、自動監視開始直後の`watch_starting`状態で停止できない問題を修正した
- `watch_starting`中に自動監視停止を許可し、`watch_starting -> stopping -> idle`の状態遷移を正規の停止経路として扱うようにした
...
- 録画画面で手動録画開始後もステータス帯が「停止中」のまま残り、再度「録画開始」を押すと「録画はすでに実行中です」と表示される問題を修正した
- 録画画面をサービスの録画状態・操作状態へ500ms間隔で同期し、開始後は録画開始を無効化、停止を有効化、自動監視中は自動監視停止へ表示を切り替えるようにした
