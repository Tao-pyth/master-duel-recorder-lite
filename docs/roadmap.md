...
実装済み: DB v10、Repository/API、`duel create` CLI、統計統合、旧JSON入出力互換。

実装継続: 戦績管理GUI、手入力追加・編集・削除、統合フィルター、GUI/E2E検証。
...
V0.2.0からV0.21.1までの中核機能と修正が完了しても、自動ではV1.0.0へ更新しません。全完了条件を確認したうえで、ユーザーが明示的に「V1.0.0に変更せよ」と依頼した場合のみ更新します。YouTube連携・アップロードとコイントス自動判定は別途計画します。
