...
- 自動監視の判定品質、診断可能性、実動画評価を継続的に改善する
- DBバックアップ、エクスポート、障害時復元の運用を強化する
- YouTube OAuth、アップロードキュー、再試行、公開範囲確認を段階的に導入する
- アクセシビリティ、キーボード操作、表示密度、長時間利用時の操作性を改善する

## V1.0.0: 中核機能完成

V0.2.0からV0.21.2までの中核機能と修正が完了しても、自動ではV1.0.0へ更新しません。全完了条件を確認したうえで、ユーザーが明示的に「V1.0.0に変更せよ」と依頼した場合のみ更新します。YouTube連携・アップロードとコイントス自動判定は別途計画します。
