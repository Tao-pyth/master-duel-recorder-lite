...
- デッキ名、タグ、シーズン画面で、一覧選択、追加、保存、削除またはアーカイブ、レポート表示を実操作化する
- テンプレート画面は投稿テンプレート編集と投稿準備キューに責務を絞り、YouTube接続管理は設定画面へ集約する
- 信頼性画面の状態更新と初回導入確認をクリック後に結果表示する
- YouTube投稿をバックグラウンド実行し、処理中、完了、失敗をUIに表示する
- ボタン、入力、プルダウン、日付入力の高さを揃え、カラー列は色コード主表示ではなく色見本で表現する
- GUI smokeと回帰テストで、固定表示、未接続、編集不能、不要ボタン再混入、バックグラウンド処理の契約を確認する
- SQLite schema、設定形式、録画ファイル、queue、manifest、OAuth資格情報は変更しない
...
デッキ・タグで色/説明を名前セルへ統合し、長い説明と多件数でも名前幅を優先する。#648。既存編集とデータ形式は維持。
