...
- コインの面を表・裏・未設定で管理する
- プレイヤー視点のコイントス勝敗を勝ち・負け・未設定で管理する
- コインの面、コイントス勝敗、先後を独立して保存し、相互推測しない
- 録画終了後と録画履歴からの後日編集に対応する
- 録画履歴へコイン情報を表示し、複合フィルターへ追加する
- 統計へコイントス条件と表裏別・勝敗別の内訳を追加する
- DBをv9へ移行し、既存記録と旧管理データは未設定で補完する
...
完了条件: v8からv9への安全な移行、独立保存と変更監査、履歴SQL複合検索、コイントス別統計、V0.19.x管理データ互換、GUI編集・表示、全単体テスト、Ruff、CLI/GUI EXEビルドと両スモークが成功すること。
...
状態: 実装済み・超横長／標準16:9実動画合格・実戦検証待ち
...
完了条件: 単一状態機械、イベント別評価、診断、通知が統合され、SHA-256固定済み超横長14本・標準16:9ウィンドウ4本のオフライン評価、初期3戦・10戦連続の実戦E2E、Ruff、全テスト、両EXE、Windows CIが成功すること。16:9で未取得のコイントス・リプレイ・マッチエラー実動画は公開時の既知リスクとして明記する。
...
追跡: [V0.25.0 Milestone](https://github.com/Tao-pyth/master-duel-recorder-lite/milestone/47)、親Issue #315、子Issue #316 - #327、修正Issue #333 - #338

完了条件: 統計ページと同じ母集団で比較・分析・推移・メモ・HTML出力を提供し、集計一致、描画、出力検証、Ruff、全テスト、両EXE、Windows CIが成功すること。

## V1.0.0: 中核機能完成

V0.2.0からV0.25.0までの計画済み中核機能と修正が完了しても、自動ではV1.0.0へ更新しません。全完了条件を確認したうえで、ユーザーが明示的に「V1.0.0に変更せよ」と依頼した場合のみ更新します。YouTube連携・アップロードはV0.25.0以降の独立計画とします。
