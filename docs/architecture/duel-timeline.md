...
V2.6.0以降で自動録画プリロールが有効な場合、`elapsed_ms` はプリロールを含む録画ファイル先頭からの経過です。開始候補 `duel_start` は0ミリ秒固定ではなく、実際に録画ファイル内で対戦開始が現れる位置へずらして保存します。DB schemaは変更せず、既存録画のイベントは従来どおり扱います。

## GUIとCLI

GUIの対戦記録詳細にタイムラインを表示し、時刻、種別、手番、勝敗、出所、信頼度、状態を確認できるようにします。状態フィルターと手動マーカー追加を提供します。この版ではイベント値の高度な編集とエクスポートは対象外です。

V2.7.4以降のPySide6レビュー画面では、タイムラインの`status`を「候補/確定/却下」の承認状態として表示し、候補行はレビュー画面から確定または却下できます。`source`は内部管理用として保持しますが、通常のレビュー表には表示しません。手動マーカーはユーザー向けに「種別」と「説明」へ分けて編集し、自動判定ラベルは検出根拠として通常編集から保護します。DB schemaは変更しません。
