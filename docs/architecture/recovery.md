...
復旧一覧は内部状態を「検査待ち」「修復可能」「修復済み」「修復不可」等の日本語で表示し、`output_empty`は「空ファイル」と表示します。修復可能な行を選択した場合だけ「修復予定」と「別ファイルへ修復」を有効にします。0バイトまたは消失済みの録画は映像データがないため修復できません。正常完了した動画の開始・終了境界の誤りもコンテナ破損ではないため、この復旧機能では変更しません。

V0.17.4確認時点の実データには復旧対象が4件あり、すべて`output_empty`のため修復不可です。GUIはこの件数と理由を表示し、実行しても成功しない修復ボタンを無効にします。

追跡: [V0.17.4 Milestone](https://github.com/Tao-pyth/master-duel-recorder-lite/milestone/34)、Issue #204
