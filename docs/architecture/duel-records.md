...
V2.7.7以降、自動監視では録画画面で選んだ自分デッキ、シーズン、希望先後を開始時点で固定し、成功した新規自動録画の`draft`へ初期値として反映できます。検出された`play_order`は希望先後より優先し、希望先後と検出先後がどちらも既知の場合だけ、希望通りなら`coin_face = heads`、希望と異なるなら`coin_face = tails`を補助入力します。これはコイントス画像の直接認識ではなく、希望先後と実際の先後の比較による補助入力です。検出先後が`unknown`の場合は`coin_face`も`unknown`のままにします。手動録画、録画なし手動戦績、候補破棄、失敗録画、既存戦績にはこの初期値を適用しません。
...
V1.0.2以降の「未完了を処理」は、連続処理画面内に勝敗、先後、自分デッキ、シーズンの簡易入力を表示します。保存後は未完了一覧を再取得し、同じ位置を基準に次の対象を表示します。詳細入力へ移動した場合も、保存またはキャンセル後に未完了処理へ戻ります。録画なし手動戦績や取込戦績では対戦日時も同じ画面で編集でき、録画付き戦績の対戦日時は録画開始日時を維持します。
...
一括編集は複数選択した既存対戦記録だけを対象にし、シーズン、自分デッキ、コインの面、対戦種別、追加タグ、削除タグを更新できます。「変更しない」は各行の既存値を保持し、「未設定」はコインの面またはシーズンを明示的に未設定値へ戻します。対象IDを全件検証してから更新し、途中失敗時は既に更新した行を元の値へ戻してエラーにします。
