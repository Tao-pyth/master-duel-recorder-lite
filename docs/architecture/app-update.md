...
取得したEXEは一時ファイルへ保存し、公開SHA-256との完全一致とGUIスモーク成功後だけ確定します。配布EXE以外では適用できません。適用時は終了後PowerShellを非表示で起動し、現EXEを`.previous`へ退避してから置換します。更新版が起動直後に異常終了した場合は旧EXEを戻して再起動します。
