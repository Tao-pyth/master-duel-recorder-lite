...
V0.7.0では録画を外部アップローダーへ渡せる状態まで、ローカルで検証・エクスポート・記述しました。V1.1.0ではこの準備結果を再利用し、YouTube公式OAuth連携とMP4自動アップロードを追加しました。V1.4.0では一般配布向けに、GUIからYouTube連携とprivate投稿を完了できる導線へ拡張しました。V1.4.1ではGUI上のMP4準備をYouTube投稿前処理として統合します。準備キュー、投稿状態、OAuth資格情報は責務を分け、秘密情報を設定、manifest、queue、標準出力、ログへ保存しません。
...
GUIでは録画IDの手入力を要求せず、完了済みで実ファイルが存在する録画を、開始日時・自分デッキ・勝敗・ファイル名を含む表示名から扱います。V1.4.1以降、MP4準備は独立した主要ナビではなく、戦績管理から開くYouTube投稿ダイアログ内の投稿前処理として表示します。内部のキュー契約は録画IDを維持します。
