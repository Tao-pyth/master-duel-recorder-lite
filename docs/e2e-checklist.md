...
26. シーズンレポートで比較、デッキ先後、コイン、日週推移、使用デッキ比率、少数標本表示が統計ページの同条件と一致することを確認する。
27. 振り返りを保存し、競合時は上書きされず、参照中シーズンがレポート確認なしに削除・アーカイブされないことを確認する。
28. 単一HTMLを出力し、印刷プレビュー、HTMLエスケープ、外部通信なし、上書き確認、出力後に開く操作を確認する。
29. `python scripts/build_windows_exe.py`、CLI・GUIのスモークが成功し、両方のPEバージョンが現在版、EXE隣接`user_data/`なし、既定先がLocalAppData配下であることを確認する。
